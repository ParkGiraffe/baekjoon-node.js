...
INPUTS.forEach((e) => {
  const [score, year, name] = e.split(" ");
  scores_names.push([+score, name]);
  years.push(year % 100);
});
...
const getFirstTeamName = () => {
  years.sort((a, b) => a - b);
  years.map((e) => e.toString().padStart(2, "0"));
  return years.join("");
};

// 성적 기반 이름
const getSecondTeamName = () => {
  scores_names.sort((a, b) => b[0] - a[0]);

  let teamName = "";
  scores_names.forEach((e) => {
    const [score, name] = e;
    teamName += name[0];
  });
