...
while (current !== inputData || isInitial) {
  if (current < 10) {
    current = parseInt("0" + current);
    // console.log(current)
  }
  let itemSumStr = current
    .toString()
    .split("")
  ...
  // console.log(curStr, itemSumStr, curStr.at(-1), itemSumStr.at(-1))

  let result = `${curStr.at(-1)}${itemSumStr.at(-1)}`;
  // console.log(result)
