...
*/
const fs = require("fs");
const [T, P] = fs.readFileSync("/dev/stdin").toString().split("\n");
//   .trim() <- 공백을 없애면 안 됨
...
const getPi = (p) => {
  ...
  const pi = new Array(pLength).fill(0);
...
const kmp = (t, p) => {
  const ans = []; // 출력할 단어 위치 저장소
  const pi = getPi(p);

  let sLength = t.length; // 전체 문자열의 길이
  let pLength = p.length; // 검색 대상 문자열의 길이
  let j = 0; // 검색대상문자열의 커서
  ...
  for (let i = 0; i < sLength; i++) {
    ...
    if (t[i] === p[j]) {
      if (j === pLength - 1) {
        // 동일한 단어를 찾은 경우
        ans.push(i - pLength + 1 + 1); // 단어를 찾은 위치를 저장, 출력할 때는 zero-based가 아니기 때문에 +1을 또 해주기
        j = pi[j];
      } else {
        j++;
      }
