...
const fs = require("fs");
const [N, STRUCT, DATA, M, INPUT] = fs.readFileSync("/dev/stdin").toString().trim().split("\n");

const n = +N;
const m = +M;

const struct = STRUCT.split(' ').map(Number);
const data = DATA.split(' ').map(Number);
const input = INPUT.split(' ').map(Number).reverse();

const deque = []; // 실상 덱과 동일한 구조
const output = [];
